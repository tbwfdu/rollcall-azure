...
class _GroupsPageState extends State<GroupsPage> {
  ...
  Future getGroups() async {
    ...
    if (m == 'google') {
      ...
      String _basicAuth = 'Basic ' + base64Encode(utf8.encode(strAuth));
      final response = await http.get(
        Uri.parse(apiurl + '/google/groups'),
        // Send authorization headers to the backend.
        headers: {
          HttpHeaders.allowHeader: 'Access-Control-Allow-Origin: *',
          //HttpHeaders.acceptHeader: 'Accept: application/json',
      ...
      for (var i = 0; i < groups.length; i++) {
        PlutoRow row = PlutoRow(
          //   sortIdx: groups[i],
          cells: {
            'column1': PlutoCell(value: groups[i]["name"]),
            'column2': PlutoCell(value: groups[i]["email"]),
            'column3': PlutoCell(value: groups[i]["id"]),
            'column4': PlutoCell(value: groups[i]["directMembersCount"]),
          },
        );
  ...
  saveSelectedGroups() async {
    ...
    //var data = [syncvalues.replaceAll(' ', '')];
    print(groups);
                ...
                Flexible(
                  child: Text(
                    mode == 'azure'
                        ...
                        : 'View user groups that exist in your Google Directory as well as those also in Workspace ONE Acccess. Also, you can use this page to Synchronise groups from Google to Workspace ONE Access by selecting the row in the Google groups list and then pressing Sync Selected.',
                    style: TextStyle(
                        fontFamily: 'SegoeUI',
                        fontSize: 14,
                  ...
                  child: needsBuildGoogle
                      ...
                      : Column(
                          children: [
                            FutureBuilder(
                              future:
                                  mode == 'google' ? getGoogleGroups() : null,
                ...
                child: Padding(
                  ...
                  child: needsBuildAzure
                      ...
                      : Column(
                          children: [
                            FutureBuilder(
                              future: mode == 'azure' ? getAzureGroups() : null,
                              builder: (BuildContext context,
                                  AsyncSnapshot snapshot) {
                                List<Widget> children;
                                if (snapshot.hasData) {
